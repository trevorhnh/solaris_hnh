...
public class Window extends Widget implements DTarget {
	...
	protected Coord ssz;
	protected boolean draw_cap = true;
	...
	static {
		...
		wbox = new IBox("gfx/hud", "tl", "tr", "bl", "br", "extvl", "extvr",
				"extht", "exthb");
	...
	protected void placecbtn() {
		cbtn.c = new Coord(wsz.x - 3 - Utils.imgsz(cbtni[0]).x, 3).add(mrgn
				.inv().add(wbox.tloff().inv()));
		fbtn.c = new Coord(cbtn.c.x - 1 - Utils.imgsz(fbtni[0]).x, cbtn.c.y);
	}

	public Window(Coord c, Coord sz, Widget parent, String cap, Coord tlo,
			Coord rbo) {
		...
		asz = new Coord(wsz.x - wbox.bl.sz().x - wbox.br.sz().x - mrgn.x, wsz.y
				- wbox.bt.sz().y - wbox.bb.sz().y - mrgn.y);
		placecbtn();
		setfocustab(true);
	...
	public void draw(GOut og) {
		...
		// Fill the background with the custom color from Config
		g.chcolor(Config.uiColor); // Set the color
		g.frect(new Coord(0, 0), wsz); // Fill the entire area with the color
		g.chcolor(); // Reset the color
