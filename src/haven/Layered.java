...
public class Layered extends Drawable {
	...
	public Map<Indir<Resource>, Sprite> sprites = new TreeMap<Indir<Resource>, Sprite>();
	...
	public static class Layer {
		...
		public Tex ol() {
			if (ol == null)
				ol = new TexI(Utils.outline(img, java.awt.Color.WHITE));
			return (ol);
		}
	...
	public static class LayerCache {
		...
		private synchronized void usecache(Object[] id) {
			...
			throw (new RuntimeException(
					"Used layered cache is not in recency list"));
		}
	...
	public synchronized void setup(Sprite.Drawer drw, final Coord cc,
			final Coord off) {
		if (base.get() == null)
			return;
		if (loading) {
