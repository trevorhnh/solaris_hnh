...
public class Resource implements Comparable<Resource>, Prioritized,
	...
	public boolean hide = false, once = false, skiphighlight = false,
			skiphide = false;
	public boolean skip_caching = false;
	static {
		try {
	...
	int prio = 0;
	//private Collection<? extends Layer> deactivated_layers = new LinkedList<Layer>();
	
	public void dumpLayers() {
		System.out.println("<layers>");
		for (Layer l : layers) {
			if (l instanceof CodeEntry) {
				System.out.println("CodeEntry: " + ((CodeEntry)l).clnm);
			} else if (l instanceof Code) {
				System.out.println("Code: " + ((Code)l).name);
				Field[] classfields = ((Code)l).getClass().getFields();
				for(Field f : classfields) {
					Class ftype = f.getType();
					System.out.println("Name: "+f.getName()+"; Type: "+ftype.getName());
				}
			} else {
	...
	public static Resource fromFile(String name) {
		Loader fLodader = new Loader(new FileSource(new File("./res")));
		Resource r = new Resource(name, -1);
		fLodader.load(r);
		return r;
	}
	
	public static Resource fromURL(String name, URL baseurl) {
		Loader fLodader = new Loader(new HttpSource(baseurl));
		Resource r = new Resource(name, -1);
		fLodader.load(r);
		return r;
	}
	
	public static Resource fromURL(URL baseurl, String name) {
		Loader fLodader = new Loader(new HttpDirectSource(baseurl));
		Resource r = new Resource(name, -1);
		fLodader.load(r);
		return r;
	}
	...
	public void addLayer(Layer... l) {
		List<Layer> lrs = new LinkedList<Layer>();
		for (Layer ll : l) lrs.add(ll);
		for (Layer lll : layers) lrs.add(lll);
		layers = lrs;
	}
	...
	public void removePagina() {
		List<Layer> lrs = new LinkedList<Layer>();
		for (Layer l : layers) {
			if (l instanceof Pagina) {
				lrs.add(l);
			}
		}
		for (Layer l : lrs) layers.remove(l);
	}
	
	public void addLayer(Resource r) {
		List<Layer> lrs = new LinkedList<Layer>();
		for (Layer ll : r.layers) lrs.add(ll);
		for (Layer lll : layers) lrs.add(lll);
		layers = lrs;
	}
	...
	public static Resource load(String name, int ver, int prio) {
		Resource res;
		if (name.startsWith(CustomMenu.menu_prefix)) {
			synchronized (CustomMenu.pagina_cache) {
				res = CustomMenu.pagina_cache.get(name);
			}
			return res;
		}
		synchronized (cache) {
			...
			if (res != null) {
				if ((res.ver != -1) && (ver != -1)) {
					if (res.ver < ver) {
						...
						// TODO ohlol kaka throws one more time!
	...
	public static class HttpDirectSource implements ResSource, Serializable {
		public URL baseurl;

		public HttpDirectSource(URL url) {
			this.baseurl = url;
		}
	...
	public class Image extends Layer implements Comparable<Image> {
		...
		public Image(Image old, BufferedImage imgn) {
			z = old.z;
			subz = old.subz;
		...
		public synchronized Tex newTex() {
			return new TexI(img);
		}
	...
	public class AButton extends Layer {
		...
		public AButton(String _name, Resource _parent, String[] _ad, char _hk) {
			name = _name;
			parent = _parent;
			hk = _hk;
			ad = _ad;
				...
				synchronized (ipe) {
					if (ipe.get(acl) != null) {
						...
						if (entry.instancer() != PublishedCode.Instancer.class)
							inst = cl.cast(entry.instancer().newInstance()
									.make(acl));
						else
	...
	private void load(InputStream in) throws IOException {
		//Resource res = Resource.load("gfx/tiles/fen/flavobjs/f10");
		...
		if (name.contains("semi")) this.ver = 5;
		...
		// UGLY SCENTS HACK
		if (Config.no_scent_smoke && name.contains("gfx/terobjs/clue")) {
	...
	private void checkerr() {
		if (error != null) {
			System.out.printf("Delayed error in resource %s (ver %d), from %s\n", name, ver, source);
...
//					+ " (v" + ver + "), from " + source, error));//
	...
	public static void dumplist(Collection<Resource> list, Writer dest) {
		...
		for (Resource res : sorted) {
			...
			if (!res.skip_caching) out.println(res.name + ":" + res.ver);
		}
	}
}
