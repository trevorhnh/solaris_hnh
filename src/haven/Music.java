...
public class Music {
	...
	Resource.addurl(new java.net.URL("https://legacy.havenandhearth.com/res/"));
	debug = true;
    ...
    private static Resource alert_sound;
    public static int startPlayBeep() {
    	play(alert_sound, true);
    	return 1;
    }
    
    public static int stopPlayBeep() {
    	play(alert_sound, true);
    	synchronized (player) {
			player.loop = false;
		}
    ...
    static {
    	try {
    		alert_sound = Resource.fromFile("sound/alert");
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
