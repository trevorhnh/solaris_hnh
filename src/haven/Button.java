...
public class Button extends Widget {
	...
	static Text.Foundry tf = new Text.Foundry(
			new Font("Serif", Font.PLAIN, 12), Color.WHITE);
	boolean a = false;
	...
	static {
		Widget.addtype("btn", new WidgetFactory() {
			public Widget create(Coord c, Widget parent, Object[] args) {
				return (new Button(c, (Integer) args[0], parent,
						(String) args[1]));
			}
		});
	...
	public void draw(GOut g) {
		synchronized (this) {
			...
			g.image(bb, new Coord(3, sz.y - bb.sz().y),
					new Coord(sz.x - 6, bb.sz().y));
